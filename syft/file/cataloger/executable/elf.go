...
func findELFSecurityFeatures(reader unionReader) (*file.ELFSecurityFeatures, error) {
...
func hasAnyDynamicSymbols(file *elf.File, symbolNames ...string) *bool {
	...
	if err != nil {
		// TODO: known-unknowns
		log.WithFields("error", err).Warnf("unable to read dynamic symbols from elf file: %w", err)
		return nil
	}
...
func hasElfDynFlag(f *elf.File, flag elf.DynFlag) bool {
	...
	if err != nil {
		// TODO: known-unknowns
		log.WithFields("error", err).Warnf("unable to read DT_FLAGS from elf file: %w", err)
		return false
	}
...
func hasElfDynFlag1(f *elf.File, flag elf.DynFlag1) bool {
	...
	if err != nil {
		// TODO: known-unknowns
		log.WithFields("error", err).Warnf("unable to read DT_FLAGS_1 from elf file: %w", err)
		return false
	}
	for _, val := range vals {
		if val&uint64(flag) != 0 {
...
func checkLLVMControlFlowIntegrity(file *elf.File) *bool {
	...
	if err != nil {
		// TODO: known-unknowns
		log.WithFields("error", err).Warnf("unable to read symbols from elf file: %w", err)
		return nil
	}
